...
class PollinationsService:
    ...
    async def generate_arrangement_image(self, db: Session, arrangement_id: str, optimized_prompt: str):
        ...
        try:    
            ...
            # NOTE: PIL often cannot open SVG in this environment.
            # Use a pre-generated PNG if available; otherwise skip logo.
            png_path = apps_dir / "web" / "src" / "assets" / "EstingsLogo.png"

            if not png_path.exists():
                print(f"⚠️ Warning: EstingsLogo.png not found at {png_path}. Skipping logo.")
            else:
                logo_img = Image.open(str(png_path)).convert("RGBA")
                # Resize so it doesn't cover the whole image
                logo_img.thumbnail((150, 150))
                # Paste the logo with transparency.
                img.paste(logo_img, (20, 20), logo_img)
        except Exception as e:
            print(f"⚠️ Error applying logo: {e}")
