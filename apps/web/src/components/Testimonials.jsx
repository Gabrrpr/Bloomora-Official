...
// Review cards keep their dark green gradient in both modes — looks great in dark
function ReviewCard({ review }) {
  return (
    <div className="flex flex-col h-full" style={{ paddingTop:"52px" }}>
      <div className="relative flex flex-col flex-1 rounded-2xl text-center"
        style={{ background:"linear-gradient(160deg,#1a5e2a 0%,#398b46 60%,#1e6b30 100%)", boxShadow:"0 8px 32px rgba(12,87,62,0.25)" }}>
        <div className="absolute left-1/2 rounded-full border-4 border-white shadow-xl overflow-hidden"
          style={{ top:"-44px", transform:"translateX(-50%)", width:"88px", height:"88px" }}>
          <AvatarIcon/>
        </div>
        <div className="flex flex-col flex-1 px-6 pt-6 pb-6">
          <div className="mb-0" style={{ fontSize:"72px", lineHeight:1, color:"rgba(255,255,255,0.35)", fontFamily:"Georgia,serif", marginTop:"40px" }}>
            &ldquo;
          </div>
          <p className="text-sm leading-relaxed flex-1" style={{ color:"rgba(255,255,255,0.92)" }}>{review.text}</p>
          <div className="mt-6">
            ...
            <div className="flex justify-center mt-3"><Stars count={review.rating}/></div>
          </div>
        </div>
      </div>
    </div>
...
export default function Testimonials() {
  ...
  const sectionBg  = isDark ? "#111827" : "#f2f8f3"
  const headingC   = isDark ? "#f3f4f6" : "#1f2937"
  const subC       = isDark ? "#9ca3af" : "#6b7280"
  const accentG    = isDark ? "#4ade80" : G
  const arrowBg    = isDark ? "#1a2332" : "white"
  const arrowBdr   = isDark ? "#4ade80" : G
  const arrowColor = isDark ? "#4ade80" : G
  const dotInactive = isDark ? "#2d3748" : "#c6e6cb"
  const videoBg    = isDark ? "#1a2332" : "white"
  const videoBdr   = isDark ? "#2d3748" : "#dceee0"
  const videoHeadC = isDark ? "#f3f4f6" : "#1f2937"
  const videoSubC  = isDark ? "#9ca3af" : "#6b7280"
  const tagBg      = isDark ? "rgba(74,222,128,0.12)" : "#e6f4ea"

  return (
    <section className="py-16 px-4 sm:px-8" style={{ backgroundColor: sectionBg }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div ref={headerRef} className="text-center mb-10"
          ...
          <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: accentG }}>Customer Reviews</p>
          <h2 className="text-3xl sm:text-4xl font-bold mb-2" style={{ color: headingC }}>What Our Customers Say</h2>
          <p className="text-sm mb-4" style={{ color: subC }}>Real feedback from real customers.</p>
          <div className="w-12 h-0.5 mx-auto" style={{ backgroundColor: accentG }}/>
        </div>
        ...
        <div ref={cardsRef}
          ...
          <div className="flex items-stretch gap-3 sm:gap-4">
            <button onClick={prev}
              ...
              style={{ backgroundColor:arrowBg, border:`2px solid ${arrowBdr}`, color:arrowColor }}>
          ...
          <div className="flex justify-center gap-2 mt-6">
            {REVIEWS.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)} className="rounded-full transition-all duration-200"
                style={{ width:i===current?"24px":"8px", height:"8px", backgroundColor:i===current?accentG:dotInactive }}/>
            ))}
          </div>
        </div>
        ...
        <div ref={videoRef} className="mt-14"
          ...
          <div className="rounded-3xl overflow-hidden shadow-xl"
            style={{ border:`1px solid ${videoBdr}`, backgroundColor:videoBg }}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 p-8 sm:p-10 pb-6">
              <div className="flex-1 min-w-0">
                <div className="inline-flex items-center px-3 py-1.5 rounded-full mb-4 w-fit" style={{ backgroundColor: tagBg }}>
                  <span className="text-xs font-bold uppercase tracking-widest" style={{ color: accentG }}>See It For Yourself</span>
                </div>
                <h3 className="text-2xl sm:text-3xl font-bold mb-3 leading-tight" style={{ color: videoHeadC }}>
                  A Tour of Our Pampanga Branch
                </h3>
                <p className="text-sm leading-relaxed" style={{ color: videoSubC }}>
