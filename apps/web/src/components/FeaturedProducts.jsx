...
function StarRating({ rating, reviews, isDark }) {
  return (
    <div className="flex items-center gap-1">
      <div className="flex items-center">
        {[1,2,3,4,5].map(i => (
          <svg key={i} className="w-3 h-3"
            fill={i <= Math.floor(rating) ? "#f59e0b" : (isDark ? "#374151" : "#e5e7eb")}
            viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
          </svg>
        ))}
      </div>
      <span className="text-xs leading-none" style={{ color: isDark ? "#6b7280" : "#9ca3af" }}>
        {rating} ({reviews})
      </span>
    </div>
  )
  ...
  return (
    <div className="absolute top-3 left-0 z-10">
      <div className="text-[10px] font-bold text-white px-3 py-1 shadow-sm"
        style={{ backgroundColor:color, clipPath:"polygon(0 0,calc(100% - 6px) 0,100% 50%,calc(100% - 6px) 100%,0 100%)", paddingRight:"16px" }}>
        {label}
      </div>
    </div>
  )
...
function ProductCard({ product, index, wishlist, toggleWishlist, discount, onPreview, isDark }) {
  ...
  const priceGreen = isDark ? "#4ade80" : G
  const origColor  = isDark ? "#6b7280" : "#9ca3af"
  const heartBg    = wishlisted ? "#fef2f2" : (isDark ? "#1e293b" : "#f9fafb")
  const heartBdr   = wishlisted ? "#fecaca" : (isDark ? "#2d3748" : "#e5e7eb")
  ...
  return (
    ...
    >
      {/* Image */}
      <div className="relative overflow-hidden" style={{ aspectRatio:"1/1", backgroundColor: isDark?"#0f172a":"#f9fafb" }}>
      ...
      <div className="p-3">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-baseline gap-2">
            <span className="text-base font-bold" style={{ color: priceGreen }}>
              ₱{product.price.toLocaleString()}
            </span>
            <span className="text-xs line-through" style={{ color: origColor }}>
          ...
          <button onClick={e => { e.stopPropagation(); toggleWishlist(product.id) }}
            ...
            style={{ backgroundColor: heartBg, border:`1px solid ${heartBdr}` }}
            title={wishlisted?"Remove from wishlist":"Add to wishlist"}>
            <svg className="w-4 h-4 transition-all duration-200"
              fill={wishlisted?"#e11d48":"none"} stroke={wishlisted?"#e11d48":"#9ca3af"} strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/>
            </svg>
          </button>
        </div>

        <p className="text-sm font-medium leading-snug mb-1.5 line-clamp-2" style={{ color: nameColor }}>
          {product.name}
        </p>

        <div className="mb-3">
          <StarRating rating={product.rating} reviews={product.reviews} isDark={isDark}/>
        </div>
        ...
        <button onClick={e => { e.stopPropagation(); onPreview(product) }}
          ...
          style={{ backgroundColor: G, borderRadius:"6px", border:"none" }}
...
export default function FeaturedProducts({ onNavigate }) {
  ...
  const viewAllBdr = isDark ? "#4ade80" : G
  const viewAllC   = isDark ? "#4ade80" : G

  return (
    <section className="py-12 px-8" style={{ backgroundColor: sectionBg }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div ref={headerRef} className="text-center mb-10"
          ...
          <h2 className="text-3xl font-bold mb-2" style={{ color: headingC }}>Featured Products</h2>
          <p className="text-sm" style={{ color: subC }}>Our most-loved flowers, all in one place.</p>
          <div className="mt-4 w-12 h-0.5 mx-auto rounded-full" style={{ backgroundColor: accentG }}/>
        </div>
        ...
        <div className="text-center mt-10">
          <button
            ...
            style={{ borderColor: viewAllBdr, color: viewAllC, borderRadius:"6px", backgroundColor:"transparent" }}
            onMouseEnter={e => { e.currentTarget.style.backgroundColor = G; e.currentTarget.style.color = "white"; e.currentTarget.style.borderColor = G; }}
            onMouseLeave={e => { e.currentTarget.style.backgroundColor = "transparent"; e.currentTarget.style.color = viewAllC; e.currentTarget.style.borderColor = viewAllBdr; }}
