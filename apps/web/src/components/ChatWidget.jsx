  ...
  return (
    <>
      <style>{`
        ...
        /* Guarantee active size button is always white regardless of dark mode CSS */
        .chat-size-active {
      ...
      <div className="fixed bottom-5 right-5 z-50 md:bottom-6 md:right-6 lg:bottom-8 lg:right-8">
        {open ? (
          ...
          >
            {/* Header — always dark green gradient, unchanged */}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3" style={{ backgroundColor: msgAreaBg }}>
              {messages.map(msg => (
                <div key={msg.id} className={`flex items-end gap-2 ${msg.from==="user"?"justify-end":"justify-start"}`}>
                  {msg.from==="bot" && (
                    <div className="w-7 h-7 rounded-lg flex-shrink-0 flex items-center justify-center mb-0.5 overflow-hidden"
              ...
              <div className="px-4 py-2.5 flex flex-wrap gap-1.5 border-t"
                ...
                {QUICK_REPLIES.map(q => (
                  <button key={q} onClick={() => sendMessage(q)}
                    ...
                    onMouseEnter={e => { e.currentTarget.style.borderColor=isDark?"#4ade80":G; e.currentTarget.style.color=isDark?"#4ade80":G; e.currentTarget.style.backgroundColor=isDark?"rgba(74,222,128,0.08)":"#f0fdf4"; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor=qrBdr; e.currentTarget.style.color=qrText; e.currentTarget.style.backgroundColor=qrBg; }}>
                    {q}
                  </button>
                ))}
              </div>
            ...
            {attachedImage && (
              <div className="px-3 pt-2 flex items-center gap-2 border-t"
                ...
                <p className="text-xs truncate" style={{ color: isDark?"#9ca3af":"#6b7280" }}>{attachedImage.file.name}</p>
              </div>
            )}
            ...
            <div className="flex items-center gap-2 px-3 py-3 border-t"
              ...
              <button onClick={handleSend}
                ...
                style={{ background: ((input.trim()||attachedImage)&&user&&sessionId&&!sending)?`linear-gradient(135deg,${DG},${G})`:(isDark?"#1e293b":"#e5e7eb") }}>
                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          // Floating button — always dark green gradient
