...
export default function DescribeArrangement({ onNavigate }) {
  ...
  return (
    ...
    <div
      ...
      style={{
        background: "radial-gradient(1100px 600px at 50% -8%, #eaf6ec 0%, #f4f9f1 45%, #fbf7ef 100%)",
      }}
    >
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="text-center mb-6">
          <p className="inline-flex items-center gap-2 text-xs sm:text-sm font-bold tracking-[0.2em] uppercase mb-2" style={{ color: G }}>
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold leading-tight mb-2" style={{ color: DG }}>
            Describe Your <span style={{ color: "#db2777" }}>Dream Bouquet</span>
          </h1>
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-6">
          ...
          <div className="space-y-5">
            ...
            {/* Prompt card */}
            <div className="bg-white/90 backdrop-blur-sm border border-green-100 rounded-3xl p-6 sm:p-7 shadow-[0_12px_40px_rgba(12,87,62,0.08)]">
              <div className="flex items-center gap-2.5 mb-1.5">
                <span className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: "rgba(46,139,52,0.1)", color: G }}>
                ...
                <h2 className="text-lg font-bold text-gray-800">Describe your arrangement</h2>
              </div>
              <p className="text-sm text-gray-400 ml-11 mb-4">Build your bouquet with just a prompt.</p>
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-5 py-4 text-base border border-gray-200 rounded-2xl focus:outline-none focus:ring-1 focus:ring-green-600 focus:border-green-600 transition resize-none placeholder-gray-300 leading-relaxed"
              />
              <div className="flex items-center justify-between mt-2.5">
                <span className="text-sm text-gray-400">{prompt.length} / {MAX}</span>
                <button
              ...
              <div className="mt-5 border-t border-gray-100 pt-4">
                <button
                  ...
                  className="flex items-center gap-2 text-sm font-semibold text-gray-600 hover:text-gray-800 transition"
                >
                  <svg
                  ...
                  <div className="mt-4 space-y-5">
                    {fetchingProducts ? (
                      ...
                      MATERIAL_CATEGORIES.map(({ key, label }) => {
                        ...
                        return (
                          <div key={key}>
                            <p className="text-sm font-semibold text-gray-600 mb-2.5">{label}</p>
                              ...
                              {categoryProducts.map(p => (
                                ...
                                >
                                  {p.image_url ? (
                                    ...
                                    <div className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center text-xs text-gray-400">No img</div>
                                  )}
                                  <span className="text-[11px] font-medium text-gray-700 leading-tight text-center truncate w-full">{p.name}</span>
                                  <span className="text-[11px] text-gray-400">₱{(+p.price).toLocaleString()}</span>
              ...
              {aiUsage && aiUsage.remaining === 0 && (
                <div className="mt-4 px-4 py-3 rounded-xl bg-red-50 border border-red-100 text-sm text-red-600">
              ...
              {aiUsage && aiUsage.remaining > 0 && aiUsage.remaining <= 2 && (
                <div className="mt-4 px-4 py-3 rounded-xl bg-amber-50 border border-amber-100 text-sm text-amber-700">
              ...
              <div className="flex items-center justify-between mt-5 gap-4">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" style={{ color: G }}>
            ...
            {unavailableItems.length > 0 && (
              <div className="bg-white border border-amber-200 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-base font-semibold text-gray-700">Some items are unavailable</h3>
                </div>
                <div className="space-y-3">
                  {unavailableItems.map(item => (
                    <div key={item.field} className="border border-gray-100 rounded-xl p-4">
                      <p className="text-sm text-gray-500 mb-1">
                        <span className="font-medium text-gray-700">{item.product_name}</span> — {item.reason}
                      </p>
                      {item.alternatives && item.alternatives.length > 0 && (
                        <div className="mt-2.5">
                          <p className="text-xs text-gray-400 mb-2">Suggested alternatives:</p>
                          <div className="flex gap-2 flex-wrap">
                            {item.alternatives.map(alt => (
                              <button
                                key={alt.product_id}
                                onClick={() => handleTryAlternative(item.field, alt.product_id)}
                                className="flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm transition-all hover:bg-green-50"
                ...
                <p className="text-sm text-gray-400 mt-3">Select an alternative above, then click "Create my bouquet" again.</p>
              </div>
            )}

            {/* Result card */}
            {result && result.success && (
              <div className="bg-white border border-green-100 rounded-3xl overflow-hidden shadow-[0_12px_40px_rgba(12,87,62,0.08)]">
                <div className="px-7 pt-6 flex items-center justify-between mb-5">
                  <div className="flex items-center gap-2.5">
                    <svg className="w-5 h-5 text-pink-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                    </svg>
                    <span className="text-base font-bold text-gray-700">Final result</span>
                    <span className="text-sm text-gray-400">Preview and analysis</span>
                  </div>
                  <button
                    onClick={() => { setResult(null); setUnavailableItems([]) }}
                    className="px-3 py-1.5 hover:bg-gray-50 rounded-lg transition text-sm text-gray-400"
                  >
                    Reset
                  </button>
                </div>
                ...
                <div className="px-7 pb-7 flex flex-col sm:flex-row gap-6">
                  {/* AI Generated Image */}
                  <div
                    className="w-full sm:w-52 h-64 rounded-2xl flex-shrink-0 flex items-center justify-center border border-gray-100 overflow-hidden bg-gray-50 cursor-pointer hover:opacity-90 transition"
                  ...
                  >
                    {result.generated_image_url ? (
                      ...
                      <div className="text-center px-3">
                        <p className="text-sm text-gray-300 mb-1">No image generated</p>
                        <p className="text-sm text-gray-400 font-medium">{arrangementName}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="mb-3">
                      <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Arrangement Name</label>
                      <input
                        type="text"
                        value={customName}
                        onChange={e => setCustomName(e.target.value)}
                        className="w-full mt-1.5 px-4 py-2.5 text-base font-semibold text-gray-800 border border-gray-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-green-600 focus:border-green-600 transition"
                        placeholder="Name your arrangement"
                      />
                    </div>
                    <p className="text-sm text-gray-500 leading-relaxed mb-5">{arrangementDesc}</p>

                    {/* Materials Used */}
                    {result.price_breakdown?.items?.length > 0 && (
                      <div className="mb-5">
                        <p className="text-sm font-semibold text-gray-700 mb-2.5">Materials Used</p>
                        <div className="flex flex-wrap gap-2 rounded-xl border border-gray-200 p-3.5">
                          {result.price_breakdown.items.map((item, idx) => (
                            ...
                            >
                              <span className="w-2 h-2 rounded-full" style={{ backgroundColor: G }} />
                              <span className="font-medium text-gray-700">{item.material_type}:</span>
                              <span className="text-gray-500">
                                {item.product_name} 
                                {item.quantity > 1 && <span className="ml-1 text-xs text-gray-400 font-semibold bg-gray-100 px-1.5 py-0.5 rounded-md">× {item.quantity}</span>}
                              </span>
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-700 mb-2.5">Cost Breakdown</p>
                        <div className="rounded-xl border border-gray-200 overflow-hidden">
                          <table className="w-full text-sm border-collapse">
                            <tbody>
                              {result.price_breakdown?.items?.map((item, idx) => (
                                <tr key={idx} className="border-b border-gray-100">
                                  <td className="px-3 py-2.5 text-gray-500 align-top">
                                    {item.product_name}
                                    {item.quantity > 1 && <span className="ml-1 text-xs text-gray-400 font-semibold bg-gray-100 px-1.5 py-0.5 rounded-md">× {item.quantity}</span>}
                                  </td>
                                  <td className="px-3 py-2.5 text-right text-gray-700 font-medium whitespace-nowrap">₱{(+item.subtotal).toLocaleString()}</td>
                                </tr>
                              ))}
                              <tr style={{ backgroundColor: "#f4f9f1" }}>
                                <td className="px-3 py-3 text-base font-bold text-gray-800">Total</td>
                                <td className="px-3 py-3 text-right text-base font-bold whitespace-nowrap" style={{ color: DG }}>₱{(+result.price_breakdown?.total_price).toLocaleString()}.00</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-700 mb-2.5">Flower Availability</p>
                          ...
                          <div className="grid grid-cols-3 gap-2 text-sm pt-1">
                            {[{ l: "Availability", s: "10/10" }, { l: "Popular", s: "9/10" }, { l: "Easy to care", s: "9/10" }].map(({ l, s }) => (
                              <div key={l} className="text-center">
                                <svg className="w-5 h-5 mx-auto mb-1" style={{ color: G }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 13l4 4L19 7" />
                                </svg>
                                <p className="font-semibold text-gray-700">{s}</p>
                                <p className="text-gray-400 text-xs leading-tight">{l}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                ...
                <div className="px-7 pb-5 flex items-start gap-2 border-t border-gray-100 pt-4">
          ...
          <div className="bg-white/90 backdrop-blur-sm border border-green-100 rounded-3xl p-7 self-start shadow-[0_12px_40px_rgba(12,87,62,0.06)]">
            <div className="flex items-center gap-2.5 mb-5">
              ...
              <h3 className="text-base font-bold text-gray-700">Prompt Tips</h3>
            </div>
                ...
                <div key={i}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                      style={{ backgroundColor: "rgba(46,139,52,0.12)", color: G }}>
                      {i + 1}
                    </span>
                    <span className="text-sm font-semibold text-gray-700">{tip.title}</span>
                  </div>
                  <p className="text-sm text-gray-500 leading-relaxed ml-8">{tip.content}</p>
                </div>
              ))}
            </div>
          </div>
    ...
    {loading && (
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
        ...
        <div className="w-full max-w-md bg-white rounded-3xl px-8 py-10 text-center shadow-2xl"
          style={{ animation: "daPop 0.3s cubic-bezier(0.34,1.56,0.64,1) both" }}>
          ...
          <h3 className="text-xl font-bold mb-1.5" style={{ color: DG }}>Creating your bouquet</h3>
          <p className="text-sm text-gray-400 mb-7">Arranging every petal just for you...</p>
          ...
          <div className="rounded-2xl px-5 py-4 text-left" style={{ backgroundColor: "#fdf2f8", border: "1px solid #fbcfe8" }}>
            ...
            <p key={factIdx} className="text-sm text-gray-600 leading-relaxed" style={{ animation: "daFade 0.5s ease both" }}>
