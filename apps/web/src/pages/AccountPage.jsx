  ...
  return (
    <div>
      ...
      <input
        ...
        onFocus={e => { if (!readOnly) e.target.style.borderColor = G; e.target.style.boxShadow = readOnly ? "none" : `0 0 0 3px ${G}18`; }}
...
// ── Overview panel ─────────────────────────────────────────────────────────────
function OverviewPanel({ user, setPanel, onNavigate }) {
  return (
...
function AddressPanel({ showToast }) {
  ...
  const [form, setForm] = useState({ label: "", street: "", city: "", province: "", zip: "" })

  const handleAdd = () => {
    if (!form.street || !form.city) return
    setAddresses(prev => [...prev, { ...form, id: Date.now() }])
    setForm({ label: "", street: "", city: "", province: "", zip: "" })
    setShowForm(false)
    ...
    <div>
      ...
      {showForm && (
        <div className="mb-6 p-5 rounded-xl" style={{ border: `1px solid ${G}40`, backgroundColor: "#f8fffe" }}>
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-4">
            <Field label="Label (e.g. Home, Office)" value={form.label}    onChange={e => setForm({ ...form, label: e.target.value })}    placeholder="Home" />
            <Field label="Street Address"             value={form.street}   onChange={e => setForm({ ...form, street: e.target.value })}   placeholder="123 Rizal St." />
            <Field label="City / Municipality"        value={form.city}     onChange={e => setForm({ ...form, city: e.target.value })}     placeholder="Manila" />
            <Field label="Province"                   value={form.province} onChange={e => setForm({ ...form, province: e.target.value })} placeholder="Metro Manila" />
            <Field label="ZIP Code"                   value={form.zip}      onChange={e => setForm({ ...form, zip: e.target.value })}      placeholder="1000" />
          </div>
          <div className="flex gap-3">
            <PrimaryBtn onClick={handleAdd}>Save Address</PrimaryBtn>
      ...
      {addresses.length === 0 && !showForm ? (
        ...
        <div className="flex flex-col gap-3">
          {addresses.map(addr => (
            <div key={addr.id} className="flex items-start justify-between p-4 rounded-xl" style={{ border: "1px solid #efefef" }}>
              <div>
                {addr.label && <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: G }}>{addr.label}</p>}
                <p className="text-sm text-gray-700">{addr.street}</p>
                <p className="text-sm text-gray-500">{addr.city}{addr.province ? `, ${addr.province}` : ""} {addr.zip}</p>
              </div>
              <button onClick={() => handleRemove(addr.id)} className="text-xs text-red-400 hover:text-red-600 transition-colors ml-4 flex-shrink-0">Remove</button>
            </div>
          ))}
        </div>
      )}
