...
function IconClock() {
  return <svg className="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.6}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
}
function IconChat() {
      ...
      <div className="relative overflow-hidden" style={{ minHeight: "260px" }}>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 sm:px-10 py-16">
          ...
          <p className="text-base max-w-xl" style={{ color: "rgba(255,255,255,0.78)" }}>
            We're here to help — visit us, give us a call, or shoot us an email. Two branches, one commitment to great service.
          </p>
        </div>
      </div>
        ...
        */}
        <div className="grid lg:grid-cols-[400px_1fr] gap-8 items-stretch">
          ...
          <div ref={leftRef} className="flex flex-col gap-5">
            ...
            {/* Branch selector */}
            <div>
              <p className="text-xs font-bold tracking-widest uppercase mb-3" style={{ color: G }}>Select Branch</p>
              <div className="flex gap-2">
                {[{ key: "manila", label: "Manila" }, { key: "pampanga", label: "Pampanga" }].map(({ key, label }) => (
                  <button key={key} onClick={() => setActiveMap(key)}
                    ...
                    style={{
                      backgroundColor: activeMap === key ? DG : "white",
                      borderColor: activeMap === key ? DG : "#e5e7eb",
            ...
            {[
              ...
              <div key={label} className="rounded-2xl p-5 flex items-start gap-4 transition-all duration-200 hover:shadow-md"
                style={{ backgroundColor: "#f8fffe", border: "1.5px solid #e0f0e8" }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: "#e6f4ea", color: DG }}>
                  {icon}
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: G }}>{label}</p>
                  {href ? (
                    <a href={href} className="text-sm font-semibold text-gray-800 hover:text-green-700 transition block break-all">{value}</a>
                  ) : (
