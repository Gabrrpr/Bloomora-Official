...
import { DG, G, StatusBadge, ExportBtn } from "./_adminShared"
    ...
    <div className="relative">
      <select
        ...
        onFocus={(e) => {
          ...
          e.target.style.boxShadow = `0 0 0 2px rgba(74,222,128,0.18)`
        }}
        onBlur={(e) => {
...
export default function AdminOrders() {
  ...
  return (
    <div className="space-y-5">
        ...
        >
          <div
            ...
            style={{
              ...
              border: `1px solid ${modalD.modalBdr}`,
              backgroundColor: modalD.modalBg,
            }}
          >
            <div
            ...
            >
              <div>
                <p className="text-base font-bold" style={{ color: isDark ? "#f1f5f9" : "#111827" }}>
                  Order Details
                </p>
            ...
            <div className="p-6 space-y-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: isDark ? "#94a3b8" : "#4b5563" }}>
                  Customer
                </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm font-bold" style={{ color: isDark ? "#4ade80" : DG }}>
                    ₱{(viewingOrder.total_amount || 0).toLocaleString()}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: isDark ? "#94a3b8" : "#4b5563" }}>
                  ...
                  <p className="text-sm font-semibold" style={{ color: isDark ? "#e2e8f0" : "#1e293b" }}>
                    {viewingOrder.created_at
                      ? new Date(viewingOrder.created_at).toLocaleDateString("en-PH", { month: "short", day: "numeric", year: "numeric" })
                      : "—"}
                  </p>
                </div>
              </div>
      ...
      {/* Heading */}
      <div>
        <p className="text-sm font-medium" style={{ color: subTxt }}>
          Your total orders
        </p>
        <div className="flex items-baseline gap-3 mt-0.5">
          <span className="text-4xl font-bold" style={{ color: isDark ? "#4ade80" : DG }}>
            {orders.length}
          </span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {STAT_CARDS.map((c) => (
          <button
            key={c.key}
            onClick={() => setStatus(statusFilter === c.key ? "All" : c.key)}
            ...
            onMouseLeave={(e) => {
              if (statusFilter !== c.key || c.green) e.currentTarget.style.transform = ""
            }}
          >
            <p className="text-xs font-semibold uppercase tracking-wider mb-1" style={{
              color: c.green ? "rgba(255,255,255,0.65)" : c.red ? "#f87171" : isDark ? "#64748b" : "#94a3b8",
            }}>
              {c.label}
            </p>
            <p className="text-xs mb-2" style={{
              color: c.green ? "rgba(255,255,255,0.5)" : c.red ? "#f87171" : isDark ? "#64748b" : "#94a3b8",
            }}>
              {c.sub}
            </p>
            <p className="text-3xl font-bold" style={{ color: c.green ? "white" : c.red ? "#f87171" : isDark ? "#4ade80" : DG }}>
              {counts[c.key]}
            </p>
          </button>
        ))}
      ...
      {(statusFilter !== "All" || branch !== "All Branches" || dateRange !== "All Time" || search) && (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-medium" style={{ color: subTxt }}>
            Active filters:
          </span>
          {statusFilter !== "All" && (
            <span
            ...
            >
              Status: {statusFilter}
              <button onClick={() => setStatus("All")} style={{ color: isDark ? "#4ade80" : "#16a34a" }}>
                ×
              </button>
            </span>
          )}
      ...
      <div
        ...
        style={{
          border: `1px solid ${isDark ? "#1e293b" : "#e8edf2"}`,
        ...
        <div className="overflow-x-auto">
          <table className="w-full" style={{ minWidth: "700px" }}>
            ...
            <tbody style={{ borderTop: `1px solid ${isDark ? "#1e293b" : "#f1f5f9"}` }}>
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-5 py-10 text-center text-sm" style={{ color: subTxt }}>
                    Loading orders...
                  </td>
                </tr>
              ) : filtered.length > 0 ? (
                filtered.map((o, idx) => (
                  <tr
                    ...
                    style={{
                      ...
                      backgroundColor: isDark ? (idx % 2 === 0 ? "#1a2332" : "#111827") : "white",
                    }}
                  ...
                  >
                    <td className="px-4 py-3">
                      <span className="font-mono text-xs" style={{ color: isDark ? "#94a3b8" : "#6b7280" }}>
                        {o.order_number}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="font-medium block" style={{ color: isDark ? "#e2e8f0" : "#1e293b" }}>
                        {o.customer_name || "—"}
                      </span>
                    ...
                    <td className="px-4 py-3">
                      <span className="font-semibold" style={{ color: isDark ? "#4ade80" : DG }}>
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-12 text-center text-sm" style={{ color: subTxt }}>
