...
export default function AdminAdvertisements() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Currently Active — full-width 3-panel card ── */}
      <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #e2e8f0", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>

        {/* Header strip */}
        <div className="flex items-center justify-between px-5 py-2.5" style={{ background: `linear-gradient(90deg, ${DG}, #1a6b4a)`, borderBottom: "none" }}>
          ...
          <div className="flex flex-col justify-between px-6 py-6">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">advertisement{activeAd.id}.png</p>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  onClick={() => setPreview(activeAd)}
                  className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-md border border-gray-200 hover:bg-gray-50 transition-all text-gray-600 w-fit"
                >
                ...
                <div className="flex flex-wrap gap-1.5">
                  {ADS.map(ad => (
                    <button
                      ...
                      style={{
                        backgroundColor: ad.id === active ? G : "#f1f5f9",
                        color: ad.id === active ? "white" : "#6b7280",
                        border: ad.id === active ? `1px solid ${G}` : "1px solid #e2e8f0",
                      }}
                    >
                      {ad.id}
                    </button>
      ...
      <div className="bg-white rounded-xl overflow-hidden" style={{ border: "1px solid #e8edf2", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: "1px solid #f1f5f9", backgroundColor: "#fafbfc" }}>
          <p className="text-sm font-semibold text-gray-800">
            All Advertisements
            <span className="ml-2 text-xs font-normal text-gray-400">{ADS.length} total</span>
          </p>
          ...
          {ADS.map(ad => {
            ...
            return (
              <div
                ...
                style={{
                  border: isActive ? `2px solid ${G}` : "2px solid #e8edf2",
                  boxShadow: isActive ? `0 0 0 3px rgba(46,139,52,0.12), 0 4px 12px rgba(0,0,0,0.06)` : "0 1px 4px rgba(0,0,0,0.04)",
              ...
              >
                ...
                <div className="relative bg-gray-50" style={{ aspectRatio: "4/3" }}>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150" style={{ background: "rgba(0,0,0,0.38)" }}>
                    <button
                      onClick={e => { e.stopPropagation(); setPreview(ad) }}
                    ...
                    {!isActive && (
                      <button
                        onClick={e => { e.stopPropagation(); handleSetActive(ad.id) }}
                ...
                <div className="px-3 py-2.5" style={{ backgroundColor: isActive ? "#f0fdf4" : "#ffffff", borderTop: `1px solid ${isActive ? "rgba(46,139,52,0.12)" : "#f1f5f9"}` }}>
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    ...
                    {isActive && (
                      <span className="flex items-center gap-1 text-[9px] font-bold" style={{ color: G }}>
                        <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse inline-block" />
                        Active
                      </span>
                    )}
      ...
      {preview && (
        ...
        >
          <div
            ...
            style={{ maxWidth: "580px", boxShadow: "0 32px 80px rgba(0,0,0,0.35)" }}
          ...
          >
            <div className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: "1px solid #f1f5f9" }}>
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">advertisement{preview.id}.png</p>
              </div>
            ...
            <div className="flex items-center justify-between px-5 py-3.5" style={{ borderTop: "1px solid #f1f5f9", backgroundColor: "#fafbfc" }}>
              {active === preview.id ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-md" style={{ backgroundColor: "#f0fdf4", color: G, border: "1px solid #bbf7d0" }}>
                  <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse inline-block" />
                  Currently Active
                </span>
              ) : (
              ...
              {active !== preview.id && (
                <button
                  onClick={() => { handleSetActive(preview.id); setPreview(null) }}
