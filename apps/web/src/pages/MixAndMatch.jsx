...
const SIZES = [
  { key: "single", label: "Single Stem", desc: "A clarity gesture.", flowers: "1 focal" },
  { key: "small", label: "Small", desc: "Sweet and simple.", flowers: "3 main flowers plus fillers", badge: "Good for gifting" },
  { key: "medium", label: "Medium", desc: "Sweet and simple.", flowers: "5 main flowers plus fillers" },
  { key: "large", label: "Large", desc: "Luxurious and full.", flowers: "8 main flowers plus fillers" },
  { key: "xl", label: "Extra Large", desc: "Ultimate statement.", flowers: "12+ main flowers" },
]

const TYPES = [
  { key: "wrapped", label: "Wrapped Arrangement", desc: "Hand bouquet, ready to give." },
  { key: "vase", label: "Vase Arrangement", desc: "Ready to display.", badge: "Ready to display" },
  { key: "box", label: "Flower in a box", desc: "Stylish box." },
  { key: "coming", label: null, desc: "We will add more soon!" },
]

const FOCAL_FLOWERS = [
  { key: "china_roses", label: "China Roses", desc: "Classic and romantic.", color: "bg-red-100", emoji: "🌹" },
  { key: "ecuador_roses", label: "Ecuador Roses", desc: "Large premium blooms.", color: "bg-pink-100", emoji: "🌸" },
  { key: "sunflower", label: "Sunflower", desc: "Bright cheerful focus.", color: "bg-yellow-100", emoji: "🌻" },
  { key: "lilies", label: "Lilies", desc: "Elegant graceful focal.", color: "bg-purple-50", emoji: "💐", outOfStock: true },
  { key: "carnation", label: "Pink Carnation", desc: "Soft sweet charm.", color: "bg-pink-50", emoji: "🌷" },
]
...
const TIPS = {
  0: [
    { title: "Small size guide", content: "This size creates a light and neat bouquet that feels balanced and easy to give.\n\nIt follows the 3-5-8 rule. You get 3 focal flowers to form the main shape, 5 greenery stems to build the structure, and 8 filler stems to add texture and detail. This combination keeps the bouquet full without making it look crowded or messy.", icon: "ℹ️" },
    { title: "What you get", items: ["3 focal flowers", "5 greenery stems", "8 fillers flowers"], icon: "✓" },
  ],
  1: [
    { title: "Wrapped arrangement", content: "Wrapped bouquets are best when you want something simple and flexible. The recipient can choose their own vase or rewrap it later. This style works well for birthdays, surprises, and casual gifts.", icon: "ℹ️" },
    { title: "Vase arrangement", content: "Vase arrangements are great for homes, offices, or events where you want the flowers to be displayed right away. This is also a good choice if the recipient does not have a vase.", icon: "ℹ️" },
    { title: "Flower in a box", content: "Box arrangements are ideal for formal gifts, special occasions, or when you want a more polished look. They also work well for delivery since the flowers are more stable.", icon: "ℹ️" },
  ],
  2: [
    { title: "Main or focal flower", content: "Roses feel romantic, sunflowers feel bright, lilies feel elegant, and carnations feel soft and sweet.", icon: "ℹ️" },
    { title: "Main or focal flower", content: "Your focal flower is the star of the arrangement. It sets the mood, color, and overall style. Pick one type to keep the bouquet clean and consistent.", icon: "ℹ️" },
    { title: "Filler flowers", content: "Filler flowers support your focal flower and make the bouquet look full and complete. They add softness, movement, and depth without taking attention away from the main flower.", icon: "ℹ️" },
  ],
  3: [
    { title: "Main or focal flower", content: "Roses feel romantic, sunflowers feel bright, lilies feel elegant, and carnations feel soft and sweet.", icon: "ℹ️" },
    { title: "Filler flowers", content: "Filler flowers support your focal flower and make the bouquet look full and complete.", icon: "ℹ️" },
  ],
}
          ...
          <div className="flex flex-col items-center">
            <div
              ...
              style={{
                ...
                backgroundColor: i < current ? G : i === current ? "white" : "white",
              }}
            >
              {i < current ? (
            ...
            <span className="text-xs mt-1 text-center max-w-[70px] leading-tight" style={{ color: i === current ? G : i < current ? "#374151" : "#9ca3af" }}>
              {step.label}
            </span>
          </div>
          {i < STEPS.length - 1 && (
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 sticky top-24">
      <div className="flex items-center gap-2 mb-4">
        <svg className="w-4 h-4 text-yellow-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2a7 7 0 00-7 7c0 2.38 1.19 4.47 3 5.74V17a1 1 0 001 1h6a1 1 0 001-1v-2.26A7 7 0 0019 9a7 7 0 00-7-7zm0 2a5 5 0 110 10A5 5 0 0112 4zM10 19h4v1a1 1 0 01-1 1h-2a1 1 0 01-1-1v-1z" /></svg>
          ...
          <div key={i}>
            ...
            {tip.items && (
              <ul className="ml-5 space-y-0.5">
                {tip.items.map(it => <li key={it} className="text-xs text-gray-500 flex items-center gap-1"><span className="text-gray-300">-</span>{it}</li>)}
              </ul>
            )}
          </div>
        ))}
...
export default function MixAndMatch({ onNavigate }) {
  ...
  const [selections, setSelections] = useState({ size: "small", type: "vase", focal: "ecuador_roses", filler: "babys_breath", ribbon: null, wrap: null })
  const [completed, setCompleted] = useState(false)

  const sel = (key, val) => setSelections(p => ({ ...p, [key]: val }))
  ...
  const RESULT = {
    ...
    availability: { available: 10, popular: 9, easyToCare: 9 },
  }
          ...
          <div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-3 text-xs">
                        {[
                          { label: "Availability", score: "10/10", icon: "M5 13l4 4L19 7" },
                          { label: "Popular", score: "9/10", icon: "M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" },
                          { label: "Easy to care", score: "9/10", icon: "M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" },
                        ].map(({ label, score, icon }) => (
                          <div key={label} className="text-center">
                            <svg className="w-4 h-4 mx-auto mb-0.5" style={{ color: G }} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={icon} /></svg>
              ...
              <div className="px-5 pb-4 flex items-start gap-2 border-t border-gray-100 pt-3">
                ...
                <p className="text-xs text-gray-400">This is an AI-generated preview. It may not reflect the exact stem count. Your bouquet will be prepared based on your selected size and options, and the price will remain the same.</p>
                <div className="flex items-center gap-1 flex-shrink-0 ml-auto">
                  <span className="text-xs text-gray-400">POWERED BY</span>
                  <span className="text-xs font-bold text-gray-500">pollinations.ai</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              {step === 0 && (
                <>
                  ...
                  <div className="grid grid-cols-5 gap-3 mb-4">
                    {SIZES.map(size => (
                      <button key={size.key} onClick={() => sel("size", size.key)}
                        className="flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all"
                        style={{ borderColor: selections.size === size.key ? G : "#e5e7eb", backgroundColor: selections.size === size.key ? "#F0F7F1" : "white" }}
                      >
                        <div className="w-16 h-16 rounded-lg bg-gradient-to-b from-pink-50 to-rose-100 flex items-center justify-center relative border border-gray-100">
                          <span className="text-xs text-gray-400">{size.label}</span>
                          {selections.size === size.key && (
                        ...
                        <span className="text-xs font-semibold text-gray-700 text-center">{size.label}</span>
                        <span className="text-xs text-gray-400 text-center leading-tight">{size.desc}</span>
                        {size.badge && (
                          <span className="text-xs px-2 py-0.5 rounded-full text-white font-medium" style={{ backgroundColor: G }}>{size.badge}</span>
                        )}
                      </button>
                    ))}
                  </div>
                  {selections.size && (
                    <div className="px-4 py-2.5 rounded-lg text-sm" style={{ backgroundColor: "#F0F7F1" }}>
                      <span className="font-medium" style={{ color: G }}>Selected size: {SIZES.find(s => s.key === selections.size)?.label}. {SIZES.find(s => s.key === selections.size)?.flowers}.</span>
                    </div>
                  )}
                </>
              )}
              ...
              {step === 1 && (
                <>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {TYPES.map((type, i) => (
                      type.key === "coming" ? (
                        <div key={i} className="flex flex-col items-center gap-2 p-3 rounded-xl border border-dashed border-gray-200 opacity-50">
                          <div className="w-20 h-20 rounded-lg bg-gray-50 flex items-center justify-center border border-gray-100">
                            <span className="text-xs text-gray-400 text-center leading-tight px-1">We will add more soon!</span>
                          </div>
                          <span className="text-xs text-gray-400 text-center">Coming soon</span>
                        </div>
                      ) : (
                        <button key={type.key} onClick={() => sel("type", type.key)}
                          className="flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all"
                          style={{ borderColor: selections.type === type.key ? G : "#e5e7eb", backgroundColor: selections.type === type.key ? "#F0F7F1" : "white" }}
                        >
                          <div className="w-20 h-20 rounded-lg bg-gradient-to-b from-pink-50 to-rose-100 flex items-center justify-center relative border border-gray-100">
                            <span className="text-xs text-gray-400 text-center leading-tight px-1">{type.label}</span>
                            {selections.type === type.key && (
                              <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center" style={{ backgroundColor: G }}>
                                <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                              </div>
                            )}
                          ...
                          <span className="text-xs text-gray-400 text-center leading-tight">{type.desc}</span>
                          {type.badge && selections.type === type.key && (
                            <span className="text-xs px-2 py-0.5 rounded-full text-white font-medium" style={{ backgroundColor: G }}>{type.badge}</span>
                          )}
                        </button>
                      )
                    ))}
              ...
              {step === 2 && (
                <>
                  ...
                  <div className="grid grid-cols-5 gap-3 mb-6">
                    {FOCAL_FLOWERS.map(f => (
                      <button key={f.key} onClick={() => !f.outOfStock && sel("focal", f.key)}
                        className="flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 transition-all relative"
                        style={{ borderColor: selections.focal === f.key ? G : "#e5e7eb", opacity: f.outOfStock ? 0.5 : 1, cursor: f.outOfStock ? "not-allowed" : "pointer" }}
                      >
                        <div className={`w-14 h-14 rounded-full ${f.color} flex items-center justify-center text-2xl border border-white`}>{f.emoji}</div>
                        <span className="text-xs font-semibold text-gray-700 text-center leading-tight">{f.label}</span>
                        <span className="text-xs text-gray-400 text-center leading-tight">{f.desc}</span>
                        {f.outOfStock && <span className="absolute top-1 right-1 text-xs bg-gray-100 text-gray-400 px-1.5 py-0.5 rounded text-center leading-tight">Out of stock</span>}
                        {selections.focal === f.key && <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center" style={{ backgroundColor: G }}><svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></div>}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <h2 className="text-sm font-semibold text-gray-800">Choose your filler flower <span className="text-gray-400 font-normal">(optional)</span></h2>
                  </div>
                  <p className="text-xs text-gray-400 ml-8 mb-4">This adds texture, shape, and volume.</p>
                  <div className="grid grid-cols-5 gap-3">
                    {FILLERS.map(f => (
                      ...
                      >
                        <div className="w-12 h-12 rounded-full bg-gray-50 flex items-center justify-center text-2xl border border-gray-100">{f.emoji}</div>
                ...
                <>
                  ...
                  <div className="flex gap-3 mb-6">
                    {[
                      ...
                      { key: "white", label: "White", color: "#f9fafb", border: true },
                      { key: "green", label: "Green", color: "#86efac" },
                      { key: "gold", label: "Gold", color: "#fde68a" },
                      { key: "none", label: "None", color: "transparent", border: true },
                    ].map(r => (
                      <button key={r.key} onClick={() => sel("ribbon", r.key)}
                      ...
                      >
                        <div
                          ...
                          style={{
                            ...
                            borderColor: selections.ribbon === r.key ? G : (r.border ? "#e5e7eb" : r.color),
                            boxShadow: selections.ribbon === r.key ? `0 0 0 2px ${G}` : "none",
                          }}
                        />
              ...
              <div className="flex items-center justify-between mt-6 pt-5 border-t border-gray-100">
                <button
                  onClick={() => step === 0 ? onNavigate("make-it-personal") : setStep(p => p - 1)}
                  className="px-5 py-2 text-sm font-medium text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50 transition"
                >
                  Return
                </button>
                <button
                  onClick={() => step < STEPS.length - 1 ? setStep(p => p + 1) : setCompleted(true)}
