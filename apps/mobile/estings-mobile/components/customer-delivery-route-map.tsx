...
export function CustomerDeliveryRouteMap({ preview }: { preview: CustomerRoutePreview }) {
  ...
  return (
    <View style={styles.card}>
      <MapLibreMap androidView="texture" attribution attributionPosition={{ bottom: 8, right: 8 }} compass logo={false} mapStyle={mapStyleUrl} style={styles.map} touchPitch={false}>
      ...
      <View style={styles.caption}><Text style={styles.captionTitle}>Planned route — not live rider location</Text><Text style={styles.captionText}>{preview.available ? routeSummary(preview) : preview.availabilityReason || 'Route line unavailable. Your verified delivery pin is still shown.'}</Text></View>
    </View>
  );
}
